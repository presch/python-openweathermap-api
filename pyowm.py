...
class OpenWeatherMapApi(object):
    ...
    def get_city_weather_by_bounding_box(self, bbox):
        ''' return a list of cities whose geographic coordinates lay within the rectangle bbox'''
    ...
    def get_station_by_coordinates_radius(self, latitude, longitude, radius):
        ''' return a list of stations whose coordinates lay within a circle, circle is defined by a center point and a radius'''
    ...
    def get_city_weather_by_coordinates_radius(self, latitude, longitude, radius):
        ''' return a list of cities whose coordinates lay witin a circle, circle is defined by a center point and a radius'''
    ...
    def get_city_weater_by_id(self, identifier):
        ''' return the current weather in a concrete chosen city where you know the city id.'''
        ...
        return City(get_list_item(city, 'id'), get_list_item(city, 'name'), get_list_item(city, 'coord'), get_list_item(city, 'distance'), 
                        get_list_item(city, 'main'), get_list_item(city, 'dt'), get_list_item(city, 'wind'), get_list_item(city, 'clouds'), 
    ...
    def get_daily_forecast(self, identifier):
        ''' return the forecast of the city for the next 7 days in a compact format by given id'''
    ...
    def get_station_list_from_dictionary(self, dictionary):
        ...
        for s_station in dictionary['list']:
            station = Station(get_list_item(s_station, 'id'), get_list_item(s_station, 'dt'), get_list_item(s_station, 'name'), 
                              get_list_item(s_station, 'type'), get_list_item(s_station, 'coord'), get_list_item(s_station, 'distance'), 
                              get_list_item(s_station, 'main'), get_list_item(s_station, 'wind'), get_list_item(s_station, 'clouds'), 
                              get_list_item(s_station, 'rain'))
    ...
    def get_city_list_from_dictionary(self, dictionary):
        ...
        for s_city in dictionary['list']:
            city = City(get_list_item(s_city, 'id'), get_list_item(s_city, 'name'), get_list_item(s_city, 'coord'), get_list_item(s_city, 'distance'), 
                        get_list_item(s_city, 'main'), get_list_item(s_city, 'dt'), get_list_item(s_city, 'wind'), get_list_item(s_city, 'clouds'), 
    ...
    def get_forecast_list_from_dictionary(self, dictionary):
        ...
        for item in get_list_item(dictionary, 'list'):
            forecast_item = ForecastItem(get_list_item(item, 'clouds'), get_list_item(item, 'snow'), get_list_item(item, 'dt_txt'), 
                                         get_list_item(item, 'weather'), get_list_item(item, 'main'), get_list_item(item, 'wind'))
            forecast_items.append(forecast_item)

        return Forecast(get_list_item(dict, 'id'), get_list_item(dict, 'city'), get_list_item(dict, 'url'), forecast_items)
    ...
    def get_compact_forecast_from_dictionary(self, dictionary):
        ...
        for item in get_list_item(dictionary, 'list'):
            compact_forecast_item = ForecastItem_compact(get_list_item(item, 'dt'), get_list_item(item, 'temp'), get_list_item(item, 'night'), 
                                                         get_list_item(item, 'eve'), get_list_item(item, 'morn'), get_list_item(item, 'pressure'), 
                                                         get_list_item(item, 'humidity'), get_list_item(item, 'weather'), get_list_item(item, 'speed'), 
                                                         get_list_item(item, 'deg'))
